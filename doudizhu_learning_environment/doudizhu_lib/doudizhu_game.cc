...
DoudizhuGame::DoudizhuGame(const GameParameters &parameters) {
  ...
  allow_aeroplane_ =
      ParameterValue<bool>(parameters_, "allow_aeroplane", false);
...
DoudizhuMove DoudizhuGame::ConstructChanceOutcome(int uid) const {
  if (uid < 0 && uid >= MaxChanceOutcomes()) {
    // Invalid.
    return {/*move_type=*/DoudizhuMove::kInvalid,
...
DoudizhuMove DoudizhuGame::ConstructMove(int uid) const {
  ...
  if (uid < kNumSolos) {
    // Solo.
    return {/*play_type=*/DoudizhuMove::PlayType::kSolo,
  ...
  if (uid < kNumPairs) {
    // Pair.
    return {/*play_type=*/DoudizhuMove::PlayType::kPair,
  ...
  if (uid < kNumTrios) {
    // Trio.
    return {/*play_type=*/DoudizhuMove::PlayType::kTrio,
  ...
  if (uid < kNumChainOfSolos) {
    ...
    const Chain chain{/*chain_type=*/ChainType::kSolo, /*length=*/chain_length,
                      /*start_rank=*/start_rank};
    return {/*play_type=*/DoudizhuMove::PlayType::kChainOfSolo,
            /*single_rank=*/{},
            /*chain=*/chain,
            /*trio_comb=*/{},
            /*quad_comb=*/{},
  ...
  if (uid < kNumChainOfPairs) {
    ...
    const Chain chain{/*chain_type=*/ChainType::kPair, /*length=*/chain_length,
                      /*start_rank=*/start_rank};
    return {/*play_type=*/DoudizhuMove::PlayType::kChainOfPair,
            /*single_rank=*/{},
            /*chain=*/chain,
            /*trio_comb=*/{},
            /*quad_comb=*/{},
  ...
  if (uid < kNumChainOfTrios) {
    ...
    const Chain chain{/*chain_type=*/ChainType::kTrio, /*length=*/chain_length,
                      /*start_rank=*/start_rank};
    return {/*play_type=*/DoudizhuMove::PlayType::kChainOfTrio,
            /*single_rank=*/{},
            /*chain=*/chain,
            /*trio_comb=*/{},
            /*quad_comb=*/{},
  ...
  if (uid < kNumBombs) {
    ...
    return {/*play_type=*/DoudizhuMove::PlayType::kBomb,
  ...
  // Rocket.
  if (uid < kNumRockets) {
    return {/*play_type=*/DoudizhuMove::PlayType::kRocket,
            /*single_rank=*/{},
            /*chain=*/{},
            /*trio_comb=*/{},
            /*quad_comb=*/{},
  ...
  uid -= kNumRockets;
  if (uid < kNumPasses) {
    return {/*play_type=*/DoudizhuMove::PlayType::kPass,
            /*single_rank=*/{},
            /*chain=*/{},
            /*trio_comb=*/{},
            /*quad_comb=*/{},
    ...
    if (play_type == DoudizhuMove::PlayType::kChainOfSolo) {
      const std::vector<int> lengths =
          Range(NumberOfChainOrPlane(kChainOfSoloMinLength),
                NumberOfChainOrPlane(kChainOfSoloMaxLength), -1);
      const int uid =
          uid_base +
    ...
    if (play_type == DoudizhuMove::PlayType::kChainOfPair) {
      const std::vector<int> lengths =
          Range(NumberOfChainOrPlane(kChainOfPairMinLength),
                NumberOfChainOrPlane(kChainOfPairMaxLength), -1);
      const int uid =
          uid_base +
          std::accumulate(lengths.begin(),
                          lengths.begin() +
    ...
    if (play_type == DoudizhuMove::PlayType::kChainOfTrio) {
      const std::vector<int> lengths =
          Range(NumberOfChainOrPlane(kChainOfTrioMinLength),
                NumberOfChainOrPlane(kChainOfTrioMaxLength), -1);
      const int uid =
          uid_base +
          std::accumulate(lengths.begin(),
                          lengths.begin() +
      ...
      const auto get_index = [&](const int length) {
        return static_cast<int>(
            std::find(plane_with_solo_per_length_.at(length).begin(),
                      plane_with_solo_per_length_.at(length).end(), move) -
      ...
      const auto get_index = [&](const int length) {
        return static_cast<int>(
            std::find(plane_with_pair_per_length_.at(length).begin(),
                      plane_with_pair_per_length_.at(length).end(), move) -

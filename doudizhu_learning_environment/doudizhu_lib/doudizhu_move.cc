    ...
    case kPlay: {
      ...
      switch (play_type_) {
        ...
        case PlayType::kRocket:
          return rv + "BR)";
        case PlayType::kSpaceShuttle:
        case PlayType::kSpaceShuttleWithSolo:
        case PlayType::kSpaceShuttleWithPair: {
          for (int i = 0; i < space_shuttle_.length; ++i) {
            for (int j = 0; j < kQuadLength; ++j) {
              rv.push_back(kRankChar[space_shuttle_.start_rank + i]);
            }
          }
          for (int rank = 0; rank < kNumRanks; ++rank) {
            for (int i = 0; i < kickers_[rank]; ++i) {
  ...
  switch (move_type_) {
    ...
    case kPlay: {
      ...
      switch (play_type_) {
        ...
        case PlayType::kSpaceShuttleWithSolo:
        case PlayType::kSpaceShuttleWithPair:
          return (space_shuttle_ == other_move.space_shuttle_ && kickers_ == other_move.kickers_);
        default:
...
bool DoudizhuMove::IsValid(const bool check_kickers) const {
  ...
  if (move_type_ == kAuction) {
    const int bid_level = static_cast<int>(auction_type_);
  ...
  if (move_type_ == kPlay) {
    switch (play_type_) {
      ...
      case PlayType::kSpaceShuttle: {
        const int kickers_count =
            std::accumulate(kickers_.begin(), kickers_.end(), 0);
        return space_shuttle_.start_rank >= 0 && space_shuttle_.length >= kSpaceShuttleMinLength
            && space_shuttle_.length <= kSpaceShuttleMaxLength && space_shuttle_.start_rank <= kNumRanks -
            space_shuttle_.length && kickers_count == 0;
      }
      case PlayType::kSpaceShuttleWithSolo: {
        const int kickers_count =
            std::accumulate(kickers_.begin(), kickers_.end(), 0);
        bool is_valid = space_shuttle_.start_rank >= 0 && space_shuttle_.length >= kSpaceShuttleMinLength
            && space_shuttle_.length <= kSpaceShuttleWithSoloMaxLength && space_shuttle_.start_rank <= kNumRanks -
            space_shuttle_.length && kickers_count == space_shuttle_.length;
        if (check_kickers) {
          for (int rank = 0; rank < kNumRanks; ++rank) {
            if (kickers_[rank] > 0) {
            }
...
std::vector<std::vector<int> > GetPossibleKickers(const QuadComb &quad_comb, const bool allow_repeated_kickers) {
    ...
    case kSolo: {
      ...
      for (int rank = 0; rank < kNumRanks; ++rank) {
        ...
        for (int another_rank = rank; another_rank < kNumRanks; ++another_rank) {
          ...
          if (another_rank == rank && !allow_repeated_kickers) {
            continue;
          }
...
std::vector<std::vector<int> > GetPossibleKickers(const Plane &plane, const bool allow_repeated_kickers) {
        ...
        for (int x_rank = 0; x_rank < kNumCardsPerSuit; ++x_rank) {
          ...
          for (int y_rank = x_rank; y_rank < kNumCardsPerSuit; ++y_rank) {
            if (IsRankInPlane(plane, y_rank))
              continue;
            if (y_rank == x_rank && !allow_repeated_kickers) {
              continue;
            }
            for (int z_rank = y_rank; z_rank < kNumCardsPerSuit; ++z_rank) {
              if ((z_rank == y_rank || z_rank == x_rank) && !allow_repeated_kickers) {
                continue;
              }
        ...
        for (int a_rank = 0; a_rank < kNumCardsPerSuit; ++a_rank) {
          ...
          for (int b_rank = a_rank; b_rank < kNumCardsPerSuit; ++b_rank) {
            if (IsRankInPlane(plane, b_rank))
              continue;
            if (b_rank == a_rank && !allow_repeated_kickers) {
              continue;
            }
            for (int c_rank = b_rank; c_rank < kNumCardsPerSuit; ++c_rank) {
              ...
              if ((c_rank == a_rank || c_rank == b_rank) && !allow_repeated_kickers) {
                continue;
              }
              for (int d_rank = c_rank; d_rank < kNumCardsPerSuit; ++d_rank) {
                if ((d_rank == a_rank || d_rank == b_rank || d_rank == c_rank) && !allow_repeated_kickers) {
                  continue;
                }
                ...
                if ((plane.start_rank > 0 &&
                    (HasKElemEqualWithTarget({a_rank, b_rank, c_rank, d_rank},
                                             3,
                                             plane.start_rank - 1))) ||
                  (plane.start_rank + plane.length < kNumCardsPerSuit - 1 &&
                    HasKElemEqualWithTarget({a_rank, b_rank, c_rank, d_rank},
              ...
              for (int d_rank = c_rank; d_rank < kNumCardsPerSuit; ++d_rank) {
                ...
                for (int e_rank = d_rank; e_rank < kNumCardsPerSuit; ++e_rank) {
                  if (IsRankInPlane(plane, e_rank))
                    continue;
                  if ((e_rank == d_rank || e_rank == c_rank || e_rank == b_rank || e_rank == a_rank) &&
                    !allow_repeated_kickers) {
                    continue;
                  }
                  const std::vector<int> rank_comb = {
                    a_rank, b_rank, c_rank,
                    d_rank, e_rank
                  };
                  if ((plane.start_rank > 0 &&
                      (HasKElemEqualWithTarget(rank_comb,
                                               3,
                                               plane.start_rank - 1))) ||
                    (plane.start_rank + plane.length < kNumCardsPerSuit - 1 &&
...
std::vector<std::vector<int> > GetPossibleKickers(const SpaceShuttle &space_shuttle) {
  switch (space_shuttle.kicker_type) {
    case kNoKicker:
      return {};
    case kSolo: {
      const int length = space_shuttle.length;
      std::vector<int> remained_ranks{};
      // Get possible ranks.
      for (int rank = 0; rank < kNumRanks; ++rank) {
        if (rank >= space_shuttle.start_rank && rank < space_shuttle.start_rank + space_shuttle.length) {
          continue;
        }
        remained_ranks.push_back(rank);
      }
      // Get all combinations.
      auto possible_combs = Combine(remained_ranks, length);
      auto contains_br = [](const std::vector<int> &comb) {
        return std::find(comb.begin(), comb.end(), kBlackJoker) != comb.end() &&
            std::find(comb.begin(), comb.end(), kRedJoker) != comb.end();
      };
      // Erase combinations contains BR.
      possible_combs.erase(std::remove_if(possible_combs.begin(),
                                          possible_combs.end(),
                                          contains_br),
                           possible_combs.end());
    ...
    case kPair: {
      ...
      const auto possible_combs = Combine(remained_ranks, space_shuttle.length);
      for (const auto &comb : possible_combs) {
        std::vector<int> temp{};
        for (const int r : comb) {
          temp.push_back(r);
    ...
    default: FatalError(
        "Error getting possible kickers for space shuttle with start_rank="
        + std::to_string(space_shuttle.start_rank) +
        "length="
        + std::to_string(space_shuttle.length) + ", "
        "kicker_type="
        + std::to_string(space_shuttle.kicker_type) + ".");
  }
}
...
std::vector<int> PlayMoveStringToRanks(const std::string &play_move_str) {
  ...
  for (const auto &c : play_move_str) {
    if (const int rank = RankCharToRank(c); rank != -1) {
      ranks.push_back(rank);
    }
  }
  return ranks;
...
DoudizhuMove GetChainMoveFromString(const std::string &move_str,
  ...
  return DoudizhuMove{
    /*chain=*/
    Chain{
      /*chain_type=*/chain_type, /*length=*/num_ranks,
...
DoudizhuMove GetDealMoveFromString(const std::string_view move_str) {
...
DoudizhuMove GetAuctionMoveFromString(const std::string_view move_str) {
  // Use 'P(p)' to identify pass.
  if (std::toupper(move_str[0]) == 'P') {
    // Pass.
    return DoudizhuMove{
      /*auction_type=*/DoudizhuMove::AuctionType::kPass
    };
  }
  ...
  return DoudizhuMove{
    /*auction_type=*/static_cast<DoudizhuMove::AuctionType>(bid)
  };
}

bool IsChain(const std::vector<int> &unique_sorted_ranks,
             const std::vector<int> &count_per_rank,
             const int num_ranks,
             const ChainType chain_type) {
...
GetChainOfSoloMoveFromRanks(const std::vector<int> &sorted_unique_ranks,
  ...
  return DoudizhuMove{
    /*chain=*/
    Chain{
      /*chain_type=*/ChainType::kSolo, /*length=*/num_ranks,
      /*start_rank=*/start_rank
    }
  };
}
...
GetChainOfPairMoveFromRanks(const std::vector<int> &sorted_unique_ranks,
  ...
  return DoudizhuMove{
    /*chain=*/
    Chain{
      /*chain_type=*/ChainType::kPair, /*length=*/num_ranks,
      /*start_rank=*/start_rank
    }
  };
}
  ...
  switch (num_cards) {
    ...
    case 5: {
      ...
      if (num_ranks == 2) {
        ...
        // Chain of solo.
        return GetChainMoveFromRanks(sorted_unique_ranks,
                                     num_ranks,
                                     ChainType::kSolo);
      }
    }
    ...
    case 6: {
      ...
      if (num_ranks == 3) {
        if (IsChain(sorted_unique_ranks,
                    count_per_rank,
                    num_ranks,
                    ChainType::kPair)) {
          return GetChainMoveFromRanks(sorted_unique_ranks,
